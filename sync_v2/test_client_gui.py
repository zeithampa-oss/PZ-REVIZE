...
def main():
 ...
 txt=tk.Text(frm,height=18); txt.pack(fill="both",expand=True,pady=(10,0))
 ...
 def sync():
  def run():
   try:
    ...
    if result.get("conflicts"): root.after(0,lambda:messagebox.showwarning("Konflikt",json.dumps(result["conflicts"],ensure_ascii=False,indent=2)))
 ...
 log("Připraveno. Spusť nejdříve testovací NAS server.")
 root.mainloop()
